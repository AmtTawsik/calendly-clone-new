<template>
    <div class="flex w-full md:h-screen justify-center items-center">
        <div :class="containerClass"
            class="rounded-md justify-center flex md:flex-row flex-col items-center md:justify-start md:shadow-xl relative">
            <client-only>
                <div v-if="isSmallDevice && selectedDate && currentTab <= 1" class="fixed top-5 left-2">
                    <button @click="relodePage" class="w-fit p-1.5 rounded-full border text-blue-500 hover:text-blue-600">
            ...
            <div>              <client-only>
                    <DatePicker @dateSelected="handleDateSelected" @dateTimeSelected="handleDateTimeSelected"
                        @personalDetails="handlePersonalDetails" @findCurrentTab="handleFindCurrentTab" :isSmallDevice="isSmallDevice"
...
const containerClass = computed(() => ({
    'md:w-[600px]': !selectedDate.value,
    'md:w-[850px]': selectedDate.value
}));
